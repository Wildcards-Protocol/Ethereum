...
contract NFT_Collection_Resolver is
    IExtendedResolver,
    Multicallable,
    ABIResolver,
    AddrResolver,
    ...
    ReverseClaimer
{
    ENS immutable ens = ENS(insert contract here);   //ENS Registry contract addr
    INameWrapper immutable nameWrapper = INameWrapper(insert contract here);   //Namewrapper contract addr
    ...
    constructor( address _nft) ReverseClaimer(ens, msg.sender) {
        tokenContract = _nft;
    }
    ...
    function resolve(bytes calldata name, bytes calldata data) public view returns(bytes memory) {
        ...
        if (resolver == address(0x0)) {
            ...
            nftContract nft = nftContract(tokenContract);

            if (functionName == 1) {
                return abi.encode(nft.ownerOf(toUint(domain)));
            }
